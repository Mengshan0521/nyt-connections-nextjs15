...
import { saveConsentStatus } from '../actions';
...
export default function ConsentDialog() {
  ...
  const handleSaveChoices = async () => {
    await saveConsentStatus(choices);
    // 强制刷新页面以应用新的同意状态
  ...
  const handleAcceptAll = async () => {
    await saveConsentStatus({ analytics: true, advertising: true });
    // 强制刷新页面以应用新的同意状态
    window.location.reload();
  };
  ...
  const handleAcceptNecessary = async () => {
    await saveConsentStatus({ analytics: false, advertising: false });
    // 强制刷新页面以应用新的同意状态
    window.location.reload();
  };
