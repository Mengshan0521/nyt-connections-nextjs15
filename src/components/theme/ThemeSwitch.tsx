...
const ThemeSwitch = () => {
  ...
  useOuterClick(() => setMenuOpen(false))

  useEffect(() => {
    const systemThemeQuery = window.matchMedia('(prefers-color-scheme: dark)');

    const updateDarkModeChecked = () => {
      if (theme === 'system') {
        setDarkModeChecked(systemThemeQuery.matches);
      } else {
        setDarkModeChecked(theme === 'dark');
      }
    };

    updateDarkModeChecked();

    systemThemeQuery.addEventListener('change', updateDarkModeChecked);

    return () => {
      systemThemeQuery.removeEventListener('change', updateDarkModeChecked);
    };
  }, [theme])
                  ...
                  <MenuItem>
                    {({ focus }) => (
                      <button
                        ...
                        } group flex w-full items-center rounded-md px-2 py-2 text-sm hover:text-blue-600 dark:hover:text-blue-400`}
                      >
